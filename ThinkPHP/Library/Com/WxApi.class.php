...
/**冰点水微信公众平台账号
 ...
 919449325@qq.com         bingdian02363324533
 * 可以不要钱
 * ******************************************************
 *
...
/* 	public function wxCache($name, $value = null) {
		if ($value == null) {
			...
			$value ['cache_deadline'] = time () + 7000;
			file_put_contents ( $name, json_encode ( $value ) );
		}
	}  */
	public function wxCache($name, $value = null) {
		if ($value == null) {
 ...
 */
	public function wxGetMaterialCount(){
		$url = "https://api.weixin.qq.com/cgi-bin/material/get_materialcount?access_token=".$this->wxAccessToken();
		return json_decode( $this->wxHttpsRequest($url),true);
	}

/**
 * 获取素材列表
 * @param  json $data {"type":"images [video] [vocie] [news]","offset":0,"count":1~20}
 * @return [type]       [description]
 */
	public function wxGetMaterialList($data)
	{
		$url = "https://api.weixin.qq.com/cgi-bin/material/batchget_material?access_token=".$this->wxAccessToken();
		return json_decode( $this->wxHttpsRequest($url,$data) ,true );
	}
 ...
 * @return array           [description]
 */
	public function wxGetPermanentMaterial(string $media_id)
	{
		$data = json_encode( array('media_id'=>$media_id) );

		$url = "https://api.weixin.qq.com/cgi-bin/material/get_material?access_token=".$this->wxAccessToken();

		return json_decode( $this->wxHttpsRequest($url,$data) , true);
	}

/**
 * 获取关注者列表  单次拉取10000条数据
 * @param  string $next_openid [description]
 * @return [type]              [description]
 */
	public function wxGetUserList($next_openid = '')
	{
		$url = "https://api.weixin.qq.com/cgi-bin/user/get?access_token=".$this->wxAccessToken()."&next_openid=".$next_openid;
		return json_decode( $this->wxHttpsRequest($url) ,true);
	}
 ...
 * @return [type] [description]
 */
	public function wxGetUserListBatch(array $data)
	{
		$param = array();
		foreach($data as $key => $openid){
			$param[$key]['openid'] = $openid;
			$param[$key]['lang']   = 'zh_CN';
		}

		$params = array('user_list'=>$param);

		$url = "https://api.weixin.qq.com/cgi-bin/user/info/batchget?access_token=".$this->wxAccessToken();

		return json_decode( $this->wxHttpsRequest($url,json_encode($params)),true );
	}
