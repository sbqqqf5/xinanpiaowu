...
<div class="modal fade" id="modal-add-cate">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="<?=U('cateList') ?>" method="POST">
...
<script>
    ...
    function update_name(obj,id)
    {
        ...
        $.post("<?=U('cateList') ?>",{"action":"name","name":value,"id":id},function(e){
            if(!e){
    ...
    function update_sorted(obj,id)
    {
        ...
        $.post("<?=U('cateList') ?>",{"action":"sorted","id":id,"sorted":value});
    }
    ...
    {
        $.post("<?=U('cateList') ?>",{"id":cur_id,"action":'del'},function(e){
            $('#'+modal_id).modal('hide');
            if(e[0]){
                $cur_tr.remove();
            }else{
...
{
    var $td = $(obj).parent();
    $.post("<?=U('cateList') ?>",{"action":"start","id":id},function(e){
...
{
    var $td = $(obj).parent();
    $.post("<?=U('cateList') ?>",{"action":"stop","id":id},function(e){
